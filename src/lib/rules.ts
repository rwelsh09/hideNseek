...
export const QUESTION_RULES: Record<string, string> = {
    "hot/cold":
        "Seekers travel a specific distance (e.g., 1km) providing the Hider with their starting and end point. The Hider must reveal if the Seekers are now Warmer (closer to) or Colder (further from) the Hider's position.",
    ...
    measure:
        "Seekers ask if the Hider is closer to or further from a specific type of place (e.g. Hospitals) than they are.",
    closest:
