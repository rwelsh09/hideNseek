...
export const QUESTION_TEXT_HANDLERS: Record<
    Question["id"],
    (questionData: any) => Promise<string> | string
> = {
    radar: (questionData) => {
        return `Are you within ${questionData.radius}${questionData.unit === "kilometers" ? "km" : "m"} of us?`;
    },
    "hot/cold": (questionData) => {
        if (
            questionData.latA &&
            questionData.lngA &&
            questionData.latB &&
...
export const getQuestionShareText = async (
    ...
    const handler = QUESTION_TEXT_HANDLERS[question.id as Question["id"]];
    if (handler) {
