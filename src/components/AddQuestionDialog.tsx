...
import {
    ...
    Waves, Target, Map as MapIcon, Ruler, Thermometer, Network, ArrowLeft
} from "lucide-react";
...
export function AddQuestionDialog() {
    ...
    // Tracks if we are currently previewing a question before committing
    const [draft, setDraft] = useState<{ id: string; key: number } | null>(null);

    // Grab the actual draft object from the store so we can render its UI
    const draftQuestion = draft ? $questions.find((q) => q.key === draft.key) : null;

    const handleOpenChange = (newOpen: boolean) => {
        // If the user closes the modal (e.g. clicking outside) while previewing, discard the draft
        if (!newOpen && draft) {
            questions.set(questions.get().filter((q) => q.key !== draft.key));
            setDraft(null);
        }
        setOpen(newOpen);
    };
    ...
    const handleQuestionSelect = (type: string, detail?: string) => {
        ...
        // Add to map immediately so preview works
    ...
    const handleSubmitDraft = () => {
        setDraft(null);
        setOpen(false); // Close dialog completely, question is already in store!
    };
    ...
    return (
        <Dialog open={open} onOpenChange={handleOpenChange}>
            ...
            {/* If in draft mode, shrink dialog to let user see the map. Otherwise wide grid. */}
            <DialogContent className={`bg-gray-100 p-0 flex flex-col max-h-[90dvh] rounded-lg overflow-hidden transition-all duration-300 ${draft ? 'max-w-sm w-[95vw]' : 'max-w-6xl w-[95vw] sm:w-full'}`}>
                
                {draftQuestion ? (
                    /* --- PREVIEW MODE UI --- */
                    <>
                        <div className="bg-slate-800 p-3 sm:p-4 m-0 shrink-0 flex items-center justify-between">
                            <Button variant="ghost" size="icon" onClick={handleCancelDraft} className="text-white hover:bg-slate-700">
                                <ArrowLeft className="w-5 h-5" />
                            </Button>
                            <DialogTitle className="text-white text-center font-bold text-lg uppercase tracking-widest m-0 pr-10">
                                Adjust Settings
                            </DialogTitle>
                        </div>
                        
                        <div className="p-4 overflow-y-auto grow flex flex-col gap-4">
                            {/* Render the actual UI component from QuestionCards based on type */}
                            ...
                            {draftQuestion.id === "thermometer" && <ThermometerQuestionComponent data={draftQuestion.data as any} questionKey={draftQuestion.key} />}
                            {draftQuestion.id === "tentacles" && <TentacleQuestionComponent data={draftQuestion.data as any} questionKey={draftQuestion.key} />}
                        </div>

                        <div className="p-4 bg-white flex justify-between gap-4 border-t shrink-0 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
                            <Button variant="outline" onClick={handleCancelDraft} className="w-full">Discard</Button>
                            <Button onClick={handleSubmitDraft} className="w-full bg-blue-600 hover:bg-blue-700 text-white">Lock In</Button>
                        </div>
                    </>
                ) : (
                    /* --- GRID MENU UI --- */
