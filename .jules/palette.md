...
## 2026-06-26 - Add text and aria-label to icon buttons in AddQuestionDialog
**Learning:** Found that relying solely on tooltips and icons for core features (like adding a question type) leads to mystery meat navigation, especially on touch devices without tooltips. Combining `flex-col` layout inside `aspect-square` buttons allows rendering both the icon and short text cleanly without changing the footprint of the layout. Also, dynamically added `aria-label` tags using python scripting are a great way to batch-update missing labels.
**Action:** When creating icon buttons that perform core actions, always consider including short, descriptive text. If space constraints demand icon-only buttons, ensure tooltips AND aria-labels are present.
