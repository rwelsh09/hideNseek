...
## $(date +%Y-%m-%d) - [Extract Inline Arrays in Map Feature Filtering]
**Learning:** `turf.distance` operations within `Array.prototype.filter` inside React components create an O(n) rendering bottleneck on every state update. While Leaflet `MapContainer` center arrays don't need memoization, computationally heavy derived values in render loops absolutely do.
**Action:** Always wrap geospatial calculations (like `turf.distance`) and expensive derived lists (like `filteredPlaces` mapped from large OpenStreetMap datasets) in `React.useMemo` to prevent significant re-render lag.
