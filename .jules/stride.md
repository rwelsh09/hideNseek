...
## 2026-07-28 - [Mobile Tap Targets and Feedback Rejected]
**Learning:** Attempted to increase critical map controls and interactive overlay triggers to a minimum of 44x44px on mobile devices, and implemented tactile touch feedback using Tailwind's `active:` pseudo-class (e.g. `active:bg-slate-200`). This mobile UI change was rejected by the owner after review.
**Action:** Do not blindly increase all map control/sidebar trigger tap targets to 44x44px or inject generic `active:` states without further context on the preferred design system or specific usability tests.
