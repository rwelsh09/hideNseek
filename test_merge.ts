...
function mergeDuplicateStationNew(places: any[], radius: number, units: turf.Units): any[] {
    // ⚡ Bolt: Use a Map of arrays to group by name efficiently, avoiding O(n^2) nested loops and array conversions
    ...
    for (const place of places) {
        ...
        let existingGroups = groupsByName.get(name);
        if (!existingGroups) {
        ...
        for (const group of existingGroups) {
            let shareZones = true; // Assume true, prove false
        ...
        if (!placeAdded) {
            existingGroups.push([place]);
        }
    }

    // 2. Compute central point per group
    const merged: any[] = [];
    for (const groups of groupsByName.values()) {
