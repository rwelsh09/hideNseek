...
describe('getQuestionShareText', () => {
    ...
    describe('hot/cold', () => {
        it('calculates distance when all coordinates are present', async () => {
            ...
            expect(result).toBe('We just moved 2.35km are we warmer or colder?');
            expect(turf.distance).toHaveBeenCalledWith([2, 1], [4, 3], { units: 'kilometers' });
        });
    ...
    describe('match', () => {
        ...
        it('handles same-first-letter-neighbourhood with boundary name', async () => {
            ...
            expect(result).toBe('Does your Neighbourhood start with the same letter as ours (D)?');
        });

        it('handles neighbourhood fallbacks when boundary errors', async () => {
            vi.mocked(determineMatchBoundary).mockRejectedValue(new Error('Failed'));
            const result1 = await getQuestionShareText({ id: 'match' }, { type: 'same-neighbourhood' });
            expect(result1).toBe('Are we in the same Neighbourhood?');

            const result2 = await getQuestionShareText({ id: 'match' }, { type: 'same-first-letter-neighbourhood' });
            expect(result2).toBe('Does your Neighbourhood start with the same letter as ours?');
        });

        it('handles other specific match types', async () => {
            expect(await getQuestionShareText({ id: 'match' }, { type: 'same-train-line' }))
                .toBe('Are you on the same Line as us?');
            expect(await getQuestionShareText({ id: 'match' }, { type: 'same-length-station' }))
                .toBe('Does your Station/Stop have the same length as ours?');
            expect(await getQuestionShareText({ id: 'match' }, { type: 'same-first-letter-station' }))
                .toBe('Does your Station/Stop start with the same letter as ours?');
        });

        it('handles generic place match', async () => {
            expect(await getQuestionShareText({ id: 'match' }, { type: 'park' }))
                .toBe('Are you near the same Park as us?');
            // Unknown place fallback
            expect(await getQuestionShareText({ id: 'match' }, { type: 'unknown_place' }))
    ...
    describe('measure', () => {
        ...
        it('calculates distance and formats for generic place', async () => {
            ...
            expect(result).toBe('We are 0.5km from a Park. Are you closer or further to your nearest Park?');
        });
    ...
    describe('closest', () => {
        it('uses plural label if available', async () => {
            const result = await getQuestionShareText({ id: 'closest' }, { locationType: 'park' });
    ...
    describe('photo', () => {
        it('uses custom notes if provided', async () => {
            const result = await getQuestionShareText({ id: 'photo' }, { notes: 'A green door' });
            expect(result).toBe('Photo challenge: A green door');
        });

        it('uses predefined description if available', async () => {
            const result = await getQuestionShareText({ id: 'photo' }, { type: 'bench' });
            expect(result).toBe('Find a bench!');
        });

        it('uses fallback text if description missing', async () => {
            const result = await getQuestionShareText({ id: 'photo' }, { type: 'fountain' });
