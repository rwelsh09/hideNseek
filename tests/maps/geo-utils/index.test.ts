...
describe("fastDistance", () => {
    it("should return 0 when the coordinates are identical", () => {
        ...
        expect(fastDistance([-50.5, 45.2], [-50.5, 45.2])).toBe(0);
    });

    it("should calculate distance correctly in kilometers", () => {
        // Equator: 0,0 to 0,1
        expect(fastDistance([0, 0], [0, 1])).toBeCloseTo(111.1950802335329, 5);
    });
    ...
    it("should be symmetrical", () => {
        ...
        expect(fastDistance(c1, c2)).toBe(fastDistance(c2, c1));
    });
    ...
    it("should handle equator and prime meridian crossings correctly", () => {
        ...
        expect(fastDistance(nw, se)).toBeCloseTo(3137.045446662887, 5);
        expect(fastDistance(se, nw)).toBeCloseTo(3137.045446662887, 5);

        // Southwest to Northeast
        const sw: [number, number] = [-10, -10];
        const ne: [number, number] = [10, 10];

        expect(fastDistance(sw, ne)).toBeCloseTo(3137.045446662887, 5);
    });
});
