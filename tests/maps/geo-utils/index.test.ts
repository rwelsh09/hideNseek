...
describe("fastDistance", () => {
    it("should return 0 when the coordinates are identical", () => {
        ...
        expect(fastDistance([10, 10], [10, 10])).toBe(0);
    });

    it("should calculate distance correctly in kilometers", () => {
        // Equator: 0,0 to 0,1
        expect(fastDistance([0, 0], [0, 1])).toBeCloseTo(
            111.1950802335329,
            5,
        );
        ...
        expect(
            fastDistance([-0.1278, 51.5074], [2.3522, 48.8566]),
    ...
    it("should be symmetrical", () => {
        ...
        expect(fastDistance(c1, c2)).toBe(
            fastDistance(c2, c1),
        );
    });
    ...
    it("should handle equator and prime meridian crossings correctly", () => {
        ...
        expect(fastDistance(nw, se)).toBeCloseTo(
            3137.045446662887,
            5,
        );
        expect(fastDistance(se, nw)).toBeCloseTo(
            3137.045446662887,
            5,
        );
        ...
        expect(fastDistance(sw, ne)).toBeCloseTo(
            3137.045446662887,
            5,
        );
    });
