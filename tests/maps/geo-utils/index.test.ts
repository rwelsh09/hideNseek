...
describe("fastDistance", () => {
    ...
    it("should calculate distance correctly in kilometers", () => {
        // Equator: 0,0 to 0,1
        expect(fastDistance([0, 0], [0, 1], "kilometers")).toBeCloseTo(111.1950802335329, 5);

        // London (-0.1278, 51.5074) to Paris (2.3522, 48.8566)
        expect(fastDistance([-0.1278, 51.5074], [2.3522, 48.8566], "kilometers")).toBeCloseTo(343.5565348808832, 5);
    });

    it("should calculate distance correctly in miles", () => {
        // Equator: 0,0 to 0,1
        expect(fastDistance([0, 0], [0, 1], "miles")).toBeCloseTo(69.093418985531, 5);

        // New York (-74.0060, 40.7128) to Los Angeles (-118.2437, 34.0522)
        expect(fastDistance([-74.0060, 40.7128], [-118.2437, 34.0522], "miles")).toBeCloseTo(2445.5626996341102, 5);
    });
    ...
    it("should handle equator and prime meridian crossings correctly", () => {
        ...
        expect(fastDistance(nw, se, "kilometers")).toBeCloseTo(3137.045446662887, 5);
        expect(fastDistance(se, nw, "kilometers")).toBeCloseTo(3137.045446662887, 5);

        // Southwest to Northeast
        const sw: [number, number] = [-10, -10];
        const ne: [number, number] = [10, 10];

        expect(fastDistance(sw, ne, "kilometers")).toBeCloseTo(3137.045446662887, 5);
    });
});
