...
async def run():
    async with async_playwright() as p:
        ...
        # Print out all text content to see what's on the page
        print("Page loaded.")
        ...
        if await add_btn.count() > 0:
            ...
            if await questions_tab.count() > 0:
                ...
                print("Could not find Add Question or Questions tab.")
        ...
        trash_buttons = await page.locator('button:has(svg)').all()
        # Find one that looks like a trash button
        # The VscTrash has path d="M14 3h-3...
